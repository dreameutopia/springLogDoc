...
5. **支配树 (Dominator Tree)**：显示堆中对象的支配关系。如果对象 A 支配对象 B，意味着所有指向 B 的路径都必须经过 A。支配树的根节点通常是占用内存最大的对象集合。通过支配树可以清晰地看到哪些对象阻止了其他大量对象的回收。  
6. **到 GC Roots 的路径 (Path to GC Roots)**：对于任何一个对象，都可以查询其到 GC Roots 的引用链。GC Roots 是垃圾收集器进行可达性分析的起点，如活动的线程、JNI 引用、JVM 系统类加载器加载的类等。如果一个不再需要的对象仍然存在到 GC Roots 的路径，就说明发生了内存泄漏。MAT 可以显示最短的引用路径，帮助定位泄漏源头。  
...
| OQL                         | 对象查询语言，类似SQL，用于查询堆内对象。                  | 灵活查询和分析堆中对象。                   |
...
3. **分析堆转储**：  
   ...
   * **类 (Classes)**：按类名显示实例数量和总大小。可以按实例数量或大小排序，快速找到占用内存最多的类。这是排查内存泄漏的常用入口。  
